...
const resolveHeadConfig = (head: DocsHeadConfig | undefined) => {
  ...
  const defaultFontStylesheetHref = fontPreset === 'inter' ? docsengineAssetUrl('fonts/fonts-inter.css') : undefined
  const defaultFontPreloadHrefs =
    fontPreset === 'inter'
      ? [
          docsengineAssetUrl('fonts/inter-v20-latin-regular.woff2'),
          docsengineAssetUrl('fonts/inter-v20-latin-600.woff2'),
          docsengineAssetUrl('fonts/inter-v20-latin-800.woff2'),
        ]
      : []
...
export const resolveDocsConfig = (config: DocsConfig): ResolvedDocsConfig => {
  if (normalizeBasePath(config.basePath) !== '/docs') {
    throw new Error(
      `docsengine currently requires basePath to be "/docs". Received ${JSON.stringify(config.basePath)}.`,
    )
  }
